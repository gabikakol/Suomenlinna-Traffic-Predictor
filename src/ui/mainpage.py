...
class MainPage:
    ...
    def start(self):
        ...
        direction_menu = ttk.Combobox(self._window, textvariable=self.dir_var, values=direction_options, width=20)
        direction_menu['values'] = direction_options
        direction_menu.grid(pady=10) 
        direction_menu.current(0)

        date = ttk.Label(self._window, text="Date", font=("Helvetica", 16))
        date.grid(column=0, padx=5, pady=10)

        #edit the funtion to set minimum date to current date
        self.cal = Calendar(self._window, mindate=datetime.date.today(), maxdate=datetime.date(2026, 1, 1))
        self.cal.grid(padx=5, pady=10)

        time = ttk.Label(self._window, text="Time (full hour)", font=("Helvetica", 16))
        ...
        hour_spinbox = ttk.Spinbox(self._window, values=hours, textvariable=self.hour_var, wrap=True, width=10)
        hour_spinbox.grid(column=0, padx=5, pady=10)

        weather = ttk.Label(self._window, text="Expected weather", font=("Helvetica", 16))
        weather.grid(column=0, padx=5, pady=10)

        temp = ttk.Label(self._window, text="1. Temperature", font=("Helvetica", 12))
        ...
        temp_menu = ttk.Combobox(self._window, textvariable=self.temp_var, values=direction_options, width=20)
        temp_menu['values'] = temp_options
        temp_menu.grid(pady=10) 
        temp_menu.current(0)

        wind = ttk.Label(self._window, text="2. Wind", font=("Helvetica", 12))
        wind.grid(column=0, padx=5, pady=10)

        wind_options = ["light wind", "moderate wind", "heavy wind"]
        self.wind_var = StringVar(self._root)
        wind_menu = ttk.Combobox(self._window, textvariable=self.wind_var, values=direction_options, width=20)
        wind_menu['values'] = wind_options
        wind_menu.grid(pady=10)
        wind_menu.current(0)

        rain = ttk.Label(self._window, text="3. Precipitation", font=("Helvetica", 12))
        ...
        rain_menu = ttk.Combobox(self._window, textvariable=self.rain_var, values=direction_options, width=20)
        rain_menu['values'] = rain_options
        rain_menu.grid(pady=10)
        rain_menu.current(0)

        save_button = ttk.Button(self._window, command=self.handle_prediction, text="Predict the traffic")
